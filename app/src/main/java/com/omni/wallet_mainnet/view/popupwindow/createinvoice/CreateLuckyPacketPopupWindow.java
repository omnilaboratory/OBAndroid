package com.omni.wallet_mainnet.view.popupwindow.createinvoice;
...
import com.omni.wallet_mainnet.client.LuckPkClient;
...
import com.omni.wallet_mainnet.thirdsupport.zxing.util.RedCodeUtils;
import com.omni.wallet_mainnet.utils.CalculateUtil;
import com.omni.wallet_mainnet.utils.CopyUtil;
import com.omni.wallet_mainnet.utils.DecimalInputTextWatcher;
import com.omni.wallet_mainnet.utils.EditInputFilter;
import com.omni.wallet_mainnet.utils.PreventContinuousClicksUtil;
import com.omni.wallet_mainnet.utils.RefConstants;
import com.omni.wallet_mainnet.utils.ShareUtil;
import com.omni.wallet_mainnet.utils.UriUtil;
import com.omni.wallet_mainnet.view.dialog.CreateChannelTipDialog;
import com.omni.wallet_mainnet.view.dialog.CreateNewChannelTipDialog;
import com.omni.wallet_mainnet.view.dialog.LoadingDialog;
import com.omni.wallet_mainnet.view.dialog.RedQRCodeDialog;
import com.omni.wallet_mainnet.view.popupwindow.SelectChannelBalancePopupWindow;
import com.omni.wallet_mainnet.view.popupwindow.SelectTimePopupWindow;
        ...
        amountEdit.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                if (!StringUtils.isEmpty(s.toString())) {
                    if (Double.parseDouble(s.toString()) - (Double.parseDouble(canUseSend)) > 0) {
//                        if (Double.parseDouble(canSend) - (Double.parseDouble(assetBalanceMax) * 0.01) == 0) {
//                            amountEdit.setText("0");
//                        } else {
                            amountEdit.setText(canUseSend);
//                        }
                    }
                }
            }
        ...
        rootView.findViewById(R.id.layout_confirm).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (PreventContinuousClicksUtil.isNotFastClick()) {
                    ...
                    if ((Double.parseDouble(amountInput) * 100000000) - (Double.parseDouble(canUseSend) * 100000000) > 0) {
                    ...
                    public void run() {
                        try {
                            ...
                            if (propertyid == 0) {
                                ...
                                canUseSend = df.format(Double.parseDouble(canSend) - (Double.parseDouble(assetBalanceMax) * 0.01));
                                mCanSendTv.setText(canUseSend);
                                mCanReceiveTv.setText(df.format(Double.parseDouble(canReceive) - (Double.parseDouble(assetBalanceMax) * 0.01)));
                            } else {
                                if (resp.getLocalBalance().getMsat() == 0) {
                                    DecimalFormat df = new DecimalFormat("0.00");
